      ...
      try {
        ...
        if (!jobDescription || !jobRole) {
          return res.status(400).json({
            success: false,
            message: "Job description and Job role is required.",
          });
        }

        const keywords=await extractKeywordsFromJd(jobDescription)

        return res.status(201).json({ success: true, keywords });
      } catch (error) {
...
export async function extractKeywordsFromJd(jobDescription,jobRole) {
  try {
